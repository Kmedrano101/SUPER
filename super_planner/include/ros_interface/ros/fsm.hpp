...
//            ros_ptr_->getSimTime(pos_cmd.header.stamp.sec, pos_cmd.header.stamp.nanosec);
    ...
    public:
        ...
        void goalCallback(const geometry_msgs::msg::PoseStamped::SharedPtr msg) {
            // Goal pose in world frame (ENU) - no axis flip needed with Gazebo + FAST-LIO world frame
        ...
        void init(const rclcpp::Node::SharedPtr nh, const std::string &cfg_path) {
            ...
            if (cfg_.timer_en) {
                ...
                // 🔧 FULL NAVIGATION MODE: Continuous replanning ENABLED
                // Replans trajectory at configured rate (default 10 Hz) during flight
                // Adapts path dynamically as drone moves and environment changes
        ...
        void pubCmdTimerCallback() {
            ...
            if (traj_finish_) {
                cout << GREEN << " -- [Fsm] Traj finish." << RESET << endl;
                // 🔧 FULL NAVIGATION MODE: Check distance to goal and regenerate if needed
                // Automatically generates new trajectory if drone moved away from goal
                if (closeToGoal(0.1)) {

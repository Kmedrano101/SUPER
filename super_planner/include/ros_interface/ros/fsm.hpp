    ...
    public:
        ...
        void init(const rclcpp::Node::SharedPtr nh, const std::string &cfg_path) {
            ...
            if (cfg_.timer_en) {
                ...
                // 🔧 QUICK START MODE: Continuous replanning disabled for simple point-to-path test
                // This allows seeing the initial path generated by SUPER without real-time updates
                // Uncomment below to enable continuous replanning (trajectory optimization during flight)
                /*
        ...
        void pubCmdTimerCallback() {
            ...
            if (traj_finish_) {
                cout << GREEN << " -- [Fsm] Traj finish." << RESET << endl;
                // 🔧 QUICK START MODE: Always go to WAIT_GOAL when trajectory finishes
                // No automatic regeneration - user must send a new goal to generate a new path
                ChangeState("PubCmdCallback", WAIT_GOAL);

                // For full navigation mode, uncomment this distance-based logic:
                /*
                if (closeToGoal(0.1)) {
                    ChangeState("PubCmdCallback", WAIT_GOAL);
                } else {
